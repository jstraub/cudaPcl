...
class PcSeg
{
  public:
    ...
    /*
     * compute pcSeg from depth image stored on the CPU
     */
    ...
    /*
     * compute pcSeg from point cloud
     */
...
PcSeg::~PcSeg()
{
  if (depthFilter_) delete depthFilter_;
};

void PcSeg::scaleDirColors(uint32_t K)
{
...
void PcSeg::compute(const pcl::PointCloud<pcl::PointXYZ>::Ptr & pc)
{
...
cv::Mat PcSeg::labelsImg(bool scaleColors)
{
  labels();
...
cv::Mat PcSeg::overlaySeg(cv::Mat img, bool showDirs, bool scaleColors)
{
  cv::Mat rgb;
  if(img.channels() == 1)
  {
