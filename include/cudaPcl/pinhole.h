...
class Pinhole {
 public:
  ...
  Pinhole(float f, uint32_t w, uint32_t h);
  ...
  Eigen::Vector3f UnprojectToCameraCosy(uint32_t i, float d) const;
  Eigen::Vector3f UnprojectToWorld(uint32_t u, uint32_t v, float d) const;
  Eigen::Vector3f UnprojectToWorld(uint32_t i, float d) const;
  ...
  const Eigen::Matrix3f GetR_W_C() const { return R_C_W_.transpose();}
  const Eigen::Vector3f& Gett_C_W() const { return t_C_W_;}
  const Eigen::Vector3f Gett_W_C() const { return -R_C_W_.transpose()*t_C_W_;}
 protected:
